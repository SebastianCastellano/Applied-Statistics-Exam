...
### question a)
plot(T,L) # we notice that observation 1 is an outlier with leverage
...
x11()
par(mfrow=c(2,2))
plot(fit) # observation 1 has cook distance > 1
...
linearHypothesis(fit, c(1,0,0), 2) # Hypothesis L0=2 satisfied

### question e)
summary(fit)

# or
linearHypothesis(fit, c(0,0,1), 0) # Hypothesis D=0 satisfied
...
plot(goat$age, goat$height, col=ifelse(goat$gender=="female",1,2), pch=20)
...
# b)
library(car)
linearHypothesis(fit,rbind(c(0,1,0,0)),0) # accepted, we could see also from summary(fit)

# c)
linearHypothesis(fit,rbind(c(0,1,0,1)),0) #rejected, there is difference
...
# e)
C <- rbind(c(1,0,0), c(1,1,0), c(1,1,1)) #respectively A, A+Bmale, A + Bfemale
