...
print(cbind(rejection.region.T2,sample.mean,ifelse(sample.mean>as.numeric(rejection.region[,2]),"Increase", "No statistical increase")))
...
S <- cov(D)
alpha <- 0.05
qT <- qt(1-alpha/k,n-1) #we don't divide alpha by 2 since we wnat to check the increase (unilateral)
rejection.region.b <- cbind("-inf", mu0 + qT * sqrt(diag(S)/n))
print(cbind(rejection.region.b,sample.mean,ifelse(sample.mean>as.numeric(rejection.region[,2]),"Increase", "No statistical increase")))